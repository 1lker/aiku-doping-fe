...
interface LeaderboardPlayer {
  ...
  const DUMMY_NAMES = [
    ...
    'Zeynep Yıldız', 'Mustafa Şahin', 'Emine Arslan', 'Hüseyin Aydın', 'Hatice Yavuz',
    'İbrahim Erdoğan', 'Elif Güneş', 'Murat Koç', 'Esra Bulut', 'Ömer Doğan',
    'Merve Özdemir', 'Emre Şen', 'Sibel Aktaş', 'Burak Yalçın', 'Derya Çetin'
  ];
  ...
  const generateInitialLeaderboard = (): LeaderboardPlayer[] => {
    return DUMMY_NAMES.map((name, index) => ({
      id: `bot-${index}`,
      name,
      avatar: `/api/placeholder/${100 + index}`,
  ...
  const simulateBotActivity = (players: LeaderboardPlayer[]): LeaderboardPlayer[] => {
    return players.map(player => {
      if (player.id.startsWith('bot-') && Math.random() < 0.3) { // 30% chance of activity
